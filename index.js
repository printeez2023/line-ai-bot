...
async function uploadToShopifyCDN(buffer, fileName, mimeType) {
  try {
    // Step 1: stagedUploadsCreate で署名付きURLを取得
    const stagedQuery = `
    ...
    const stagedRes = await fetch(`https://${SHOPIFY_DOMAIN}/admin/api/${SHOPIFY_API_VERSION}/graphql.json`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Shopify-Access-Token': SHOPIFY_ACCESS_TOKEN,
    ...
    if (stagedErrors && stagedErrors.length > 0) {
      console.error('StagedUpload取得失敗:', JSON.stringify(stagedErrors));
      return null;
    }

    const target = stagedData?.data?.stagedUploadsCreate?.stagedTargets?.[0];
    if (!target) {
      console.error('StagedUploadターゲットなし');
      return null;
    }

    // Step 2: 署名付きURLにファイルをPOSTアップロード
    const { FormData: NodeFormData } = require('node:buffer') ? { FormData: null } : {};
    ...
    if (createErrors && createErrors.length > 0) {
      console.error('ShopifyfileCreate失敗:', JSON.stringify(createErrors));
      return null;
    }
