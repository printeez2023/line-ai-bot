...
function getUser(userId) {
  if (!users.has(userId)) {
    // pendingHandoff: スタッフ引き継ぎ直前の「ご不明点確認中」状態
...
function aiOff(userId) {
  const user = getUser(userId);
  user.mode = 'staff';
  user.staffSince = null; // タイムアウト廃止のため不使用
  user.pendingHandoff = false;
...
🔗 HP：https://printeez.jp/products/ファイルアップロード用ページ」
  ...
  → ユーザーが画像ファイル（JPG・PNG等）をLINEに直接送ってきた場合も入稿とみなす
  → 「入稿完了」が選ばれた・画像が送られてきた・「スタッフを呼ぶ」が選ばれた場合：
...
【「注文する」「注文したい」と言われた場合】
お客様から「注文する」「注文したい」「お願いしたい」などの注文意思が示されたら、
以下の流れを丁寧に説明してください：

「ありがとうございます！ご注文の流れをご説明しますね😊
...
// triggerType: 'nyuukou' | 'staff_request'
  ...
  async (req, res) => {
    ...
    for (const event of events) {
      ...
      try {
        ...
        // autoHandoff: true → pre-handoff フローへ（即引き継ぎしない）
        if (parsed.autoHandoff === true) {
          // まずキキの返答を送り、その後 pre-handoff メッセージを送る
          // ※replyToken は1回しか使えないため、キキの返答内に pre-handoff を組み込む
          const summary = await generateSummary(userId);
          user.pendingHandoff = true;
