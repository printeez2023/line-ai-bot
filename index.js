...
// ===== [修正①] 全商品取得（初回セッション用・JSON形式・vendor/tags含む）=====
// all-itemsコレクション依存をやめ /products.json 直接取得に変更
async function getAllProductsWithMetafields() {
  ...
  console.log('全商品取得中（/products.json直接取得）...');

  const productList = [];
  let url = `https://${SHOPIFY_DOMAIN}/admin/api/${SHOPIFY_API_VERSION}/products.json?limit=250&published_status=published&fields=id,title,handle,vendor,tags,variants`;
  ...
  console.log(`全商品取得完了: ${productList.length}件`);
...
・計算に使った単価が商品情報の最安値であるなど、実際の金額と異なる可能性がある場合は、金額の前に「おおよそ」をつけてください。例：おおよそ AI見積 合計 12,000円
・根拠が明確な場合（商品情報に明記されている場合など）は「おおよそ」は不要ですが、「AI見積」は必ずつけてください。
...
型番（例：5001、00085）で聞かれた場合はhandleまたはtitleから一致するものを探してください。
...
async function askGemini(userId, userMessage) {
  ...
  // 初回メッセージ → 全商品一覧を注入（JSON形式・vendor/tags含む）
