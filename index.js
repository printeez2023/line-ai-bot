...
🔗 HP：https://printeez.jp/products/ファイルアップロード用ページ」
  ...
  → 「入稿完了」が選ばれた・画像が送られてきた・「スタッフを呼ぶ」が選ばれた場合：
     「お疲れさまでした！それでは、スタッフに引き継ぎますね😊」と返してから autoHandoff: true
...
app.post('/webhook',
  ...
  async (req, res) => {
    res.sendStatus(200); // LINEへ即200を返す（タイムアウト再送を防ぐ）
    ...
    for (const event of events) {
      ...
      // =========================================================
      // ① eventId を最優先でチェック＆保存
      //    → await の前に確定させることで再送の二重処理を防ぐ
      // =========================================================
      if (isProcessed(eventId)) {
        console.log(`[${userId}] 重複eventIdをスキップ: ${eventId}`);
        continue;
      }
      saveEventId(eventId); // ← await より前に即保存
      ...
      try {
        ...
        processingUsers.delete(userId); // 必ずロック解除
      }
    }
  }
);
