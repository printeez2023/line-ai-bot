...
function getUser(userId) {
  if (!users.has(userId)) {
    users.set(userId, {
      ...
      awaitingNyuukou: false, // ステップ4入稿待ち中フラグ
    });
  }
...
【見積もりの自己検証】
見積もりを出す前に、必ず以下を自分で確認してください：
1. 商品単価は商品情報の「価格：」から取得しているか
2. 加工代金は料金ページの該当区分から取得しているか（スクリーン/フルカラー/刺繍、枚数・サイズ区分）
3. 単価 × 枚数 + 送料1,000円 の計算が合っているか
4. 根拠が不明な場合は「おおよそ」をつけているか
確認後に出力してください。計算ミスがあれば訂正してから出力してください。
    ...
    for (const event of events) {
      ...
      try {
        ...
        if (
          ...
          // pendingHandoff中なら即引き継ぎ
          if (user.pendingHandoff) {
            await executeHandoff(userId, replyToken);
            continue;
          }

          // 画像・ファイルはいかなる場合もキキは確認できない
          await client.replyMessage(replyToken, {
            type: 'text',
            text: 'すみません、画像・ファイルの内容はキキには確認できません🙏\nスタッフをお呼びしますか？\n\nbyAI🦊キキ',
            quickReply: {
              items: [
        ...
        if (parsed.autoHandoff === true) {
          ...
          // nyuukouReady: true → ステップ4入稿待ち状態にセット
          if (parsed.nyuukouReady === true) {
            user.awaitingNyuukou = true;
            console.log(`[${userId}] 入稿待ち状態にセット`);
          }
          await client.replyMessage(replyToken, buildMessages(parsed));
          user.lastBotReply = Date.now();
        }
