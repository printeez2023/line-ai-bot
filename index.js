...
// ★ 修正: published_status=published を追加して非公開商品を除外
...
// ShopifyファイルGID → CDN URLに解決（すでにhttpsならそのまま返す）
...
// ★ 追加: 在庫チェック — 購入可能なバリアントが1つでもあればtrue
...
🔗 HP：https://printeez.jp/products/ファイルアップロード用ページ」
  その後、クイックリプライで「入稿完了」「入稿できない」「スタッフを呼ぶ」を出してください。PDF/PNG/AI/PSD/JPG
...

{
  ...
  "imageActions": [
    ...
    { "type": "size",  "handle": "商品のhandle名" }  ← サイズチャートがある場合のみ
  ]
}

imageActionsはシステムが画像を自動送信するためのトリガーです。
カラー表はステップ1（商品確定直後）に送信します。
サイズチャートはステップ3（数量・サイズ確認時）に送信します。
...
// imageActionsを含む場合は複数メッセージを返す
// LINE replyMessage は最大5件まとめて送れる
  ...
  async (req, res) => {
    ...
    for (const event of events) {
      ...
      try {
        ...
        // autoHandoff: true → pre-handoff フローへ
